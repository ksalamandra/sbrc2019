...
plt.plot(x, np.asarray(k)/1000, 'b-',label="Tráfego total direcioadp ap servidor", linewidth=wd)
plt.plot(x, np.asarray(y)/1000, 'r-.', label="Limitação dinâmica de banda", linewidth=wd, markersize=20, markeredgewidth=4)
#plt.plot(x, np.asarray(z)/1000, 'r', label="Cliente 3", linewidth=wd)
plt.plot(x, np.asarray(m)/1000, 'k', label="Limite de banda médio", linestyle='dotted', linewidth=wd)
#
